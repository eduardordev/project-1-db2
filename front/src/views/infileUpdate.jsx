...
export default function InfileUpdate(props) {
    console.log(props);
    ...
    return props.data.map((index) => (
        <React.Fragment key={index.id}>
            <h4>Actualizar Detalles de Factura: {index.id}</h4>
            <hr />
            <div style={{ display: "flex", flexDirection: "row", width: "100%", justifyContent: "space-between" }}>
                <div style={{ display: "flex", flexDirection: "column" }}>
                    <div>Cliente: {index.name} <button onClick={props.toggleModalName}><i className="fa fa-edit"/></button> </div>
                    <div>Nit: {index.nit} <button onClick={props.toggleModalNit}><i className="fa fa-edit" /></button> </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column" }}>
